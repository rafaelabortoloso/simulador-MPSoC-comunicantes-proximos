...
var jsonFilePath = path.join(__dirname, 'JsonCorrecao', 'Test3.json');
  ...
  for (const app of apps) {
    ...
    fs.readFile(appFile, 'utf8', (err, data) => {
      ...
      grafo_tarefas.forEach((tarefa) => {
        ...
        for (let i = 0; i < grafo_tarefas.length; i++) {

          let sourcePosition = srcExt(matriz, source);
          let targetPosition = tgtExt(matriz, target);
          ...
          if (sourcePosition) { //verifica se o source ja existe
            ...
            newSourcePosition = getVizinhos(noInicial).find((v) => {
              return matriz[v[0]][v[1]] == null;
            });
      ...
      function tgtExt(matriz, target) {
        for (let i = 0; i < matriz.length; i++) {
          for (let j = 0; j < matriz[i].length; j++) {
            if (!!matriz[i][j]) {
              if (matriz[i][j].find((value) => value == target)) {
                return [i, j];
              };
            }
          }
